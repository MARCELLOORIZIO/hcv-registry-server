...
async function main() {
  ...
  if (login.status !== 200 || !login.body?.token) {
    console.error(`SIGILLUM_KYC_TEST: LOGIN_FAILED HTTP ${login.status}`);
    console.error(login.text);
    process.exit(1);
  }
  console.log('SIGILLUM_KYC_TEST: LOGIN_OK');

  const token = login.body.token;
  const start = await request('POST', '/api/identity/kyc/start', { token, body: {} });
  if (start.status !== 200 || !start.body?.sessionId) {
    console.error(`SIGILLUM_KYC_TEST: START_FAILED HTTP ${start.status}`);
    console.error(start.text);
    process.exit(1);
  }

  console.log('SIGILLUM_KYC_TEST: VERIFICATION_SESSION_CREATED');
  console.log(JSON.stringify({
    provider: start.body.provider || 'stripe_identity',
    sessionId: start.body.sessionId,
    status: start.body.status,
    hasUrl: Boolean(start.body.url),
    url: start.body.url || '',
  }));

  const session = await request('GET', '/api/auth/session', { token });
  if (session.status === 200 && session.body?.account) {
    console.log('SIGILLUM_KYC_TEST: ACCOUNT_STATE');
    console.log(JSON.stringify({
      kycStatus: session.body.account.kycStatus,
      legalIdentityVerified: session.body.account.legalIdentityVerified,
      creatorAccess: session.body.account.creatorAccess,
    }));
  }

  const logout = await request('POST', '/api/auth/logout', { token });
  if (logout.status === 200) console.log('SIGILLUM_KYC_TEST: LOGOUT_OK');
}

main().catch(error => {
  console.error('SIGILLUM_KYC_TEST: ERROR');
