...
async function main() {
  const login = await request('POST', '/api/auth/login', {
    body: { email, password, ...deviceProof() },
  });
  if (login.status !== 200 || !login.body?.token) {
    console.error(`SIGILLUM_KYC_STATUS_TEST: LOGIN_FAILED HTTP ${login.status}`);
    console.error(login.text);
    process.exit(1);
  }
  console.log('SIGILLUM_KYC_STATUS_TEST: LOGIN_OK');
  const token = login.body.token;

  const kyc = await request('GET', '/api/identity/kyc/status', { token });
  if (kyc.status !== 200) {
    console.error(`SIGILLUM_KYC_STATUS_TEST: STATUS_FAILED HTTP ${kyc.status}`);
    console.error(kyc.text);
    process.exit(1);
  }
  console.log('SIGILLUM_KYC_STATUS_TEST: STRIPE_REFRESH_OK');
  console.log(JSON.stringify({
    status: kyc.body.status,
    verified: kyc.body.verified,
    sessionId: kyc.body.sessionId || '',
    legalNamePresent: Boolean(kyc.body?.verifiedOutputs?.legalName),
    country: kyc.body?.verifiedOutputs?.country || '',
    lastError: kyc.body.lastError || null,
  }));

  const session = await request('GET', '/api/auth/session', { token });
  if (session.status !== 200 || !session.body?.account) {
    console.error(`SIGILLUM_KYC_STATUS_TEST: ACCOUNT_FAILED HTTP ${session.status}`);
    console.error(session.text);
    process.exit(1);
  }
  const account = session.body.account;
  console.log('SIGILLUM_KYC_STATUS_TEST: ACCOUNT_STATE');
  console.log(JSON.stringify({
  ...
  if (logout.status === 200) console.log('SIGILLUM_KYC_STATUS_TEST: LOGOUT_OK');
}

main().catch(error => {
  console.error('SIGILLUM_KYC_STATUS_TEST: ERROR');
