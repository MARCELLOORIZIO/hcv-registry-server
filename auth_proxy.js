...
const AUTH_UPSTREAM_PORT = Number(
  process.env.AUTH_UPSTREAM_PORT || PUBLIC_PORT + 1,
);
const REGISTRY_LEGACY_PORT = Number(
  process.env.AUTH_LEGACY_PORT || PUBLIC_PORT + 2,
);

// Run the existing KYC + certificate Registry unchanged behind this gateway.
const originalPort = process.env.PORT;
const originalLegacyPort = process.env.LEGACY_PORT;
process.env.PORT = String(AUTH_UPSTREAM_PORT);
process.env.LEGACY_PORT = String(REGISTRY_LEGACY_PORT);
require('./kyc_proxy.js');
if (originalPort == null) delete process.env.PORT;
else process.env.PORT = originalPort;
if (originalLegacyPort == null) delete process.env.LEGACY_PORT;
else process.env.LEGACY_PORT = originalLegacyPort;
...
const publicServer = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/health') {
      return sendJson(res, 200, {
        ok: true,
        service: 'hcv-registry-auth-gateway',
    ...
    return proxyToRegistry(req, res);
  } catch (error) {
    return sendJson(
      res,
      error.statusCode || 500,
      error.statusCode ? publicError(error) : {
        ok: false,
        error: 'ERRORE_SERVER',
...
publicServer.listen(PUBLIC_PORT, '0.0.0.0', () => {
  console.log(
    `SIGILLUM auth gateway listening on http://0.0.0.0:${PUBLIC_PORT}; KYC/Registry upstream on 127.0.0.1:${AUTH_UPSTREAM_PORT}`,
  );
});
